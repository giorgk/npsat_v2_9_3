...
#include "npsat_trace/traj_atlas.h"
...
public:
  ...
  void run();
private:
  void run_point_tracing();
  void run_trajectory_atlas();
  ...
  npsat_trace::CellTrajectoryAtlas<dim> &get_or_build_cell_atlas(
    const typename DoFHandler<dim>::active_cell_iterator &cell,
  ...
  unsigned int atlas_owner_for_position(const Point<dim> &position) const;
  void exchange_atlas_packets(
  ...
  std::vector<npsat_trace::CellTrajectoryAtlas<dim> > all_cell_atlases;
  std::vector<bool> all_cell_atlas_valid;
  std::vector<unsigned int> all_cell_atlas_flow_step;
...
#include "npsat_trace/main_class_impl/npsat_trace_point.impl.h"
#include "npsat_trace/main_class_impl/npsat_trace_atlas.impl.h"


#if 0
// The legacy point implementation was moved verbatim to
// npsat_trace_point.impl.h.  This disabled copy is retained temporarily to
// keep the extraction reviewable against the original source history.
template <int dim>
...
void NPSAT_TRACE<dim>::run()
{
  if (topt.sim_opt.transport_method == npsat_trace::TransportMethod::trajectory_atlas)
